...
testCases.forEach((platform) => {
  describe(`Testing ${platform}`, () => {
    ...
    test("resolve with a detailed valid configuration", () => {
      return start({ ...baseConfig }).then((result) =>
    ...
    test("resolve an empty captureDocument object", () => {
      return start({
        ...baseConfig,
        flowSteps: { ...flowSteps, captureDocument: {} },
    ...
    test("resolve a incorrect config attribute type", () => {
      return start({
        ...baseConfig,
        flowSteps: { ...flowSteps, captureDocument: true },
      } as unknown as OnfidoConfig).then((result) =>
        expect(result).toBe(RESOLVED)
      );
    });
    ...
    test("resolve a capture document object with explicitly null attributes", () => {
      return start({
        ...
        flowSteps: {
          ...flowSteps,
          captureDocument: { docType: null, countryCode: null },
        },
      } as unknown as OnfidoConfig).then((result) =>
        expect(result).toBe(RESOLVED)
      );
    ...
    test("resolve with a valid workflow runId", () => {
      return start({ ...baseConfig, workflowRunId: workflowRunId }).then(
        (result) => expect(result).toBe(RESOLVED)
      );
    });
    ...
    test("reject a null config object", () => {
      return start(null as unknown as OnfidoConfig).then((result) =>
    ...
    test("reject with countryCode specified but no docType", () => {
      return start({
        ...
        flowSteps: {
          ...flowSteps,
          captureDocument: { countryCode: OnfidoCountryCode.GBR },
        },
      }).then((result) => expect(result).toBe(REJECTED));
    });
    ...
    test("reject with an invalid docType", () => {
      return start({
        ...
        flowSteps: {
          ...flowSteps,
          captureDocument: { docType: OnfidoCountryCode.GBR },
        },
      } as unknown as OnfidoConfig).then((result) =>
        expect(result).toBe(REJECTED)
      );
    ...
    test("reject with an invalid capture type", () => {
      return start({
        ...
        flowSteps: {
          ...flowSteps,
          captureFace: OnfidoDocumentType.DRIVING_LICENCE,
        },
      } as unknown as OnfidoConfig).then((result) =>
        expect(result).toBe(REJECTED)
      );
    ...
    test("base 64 helper function converts data array correctly to base64", () => {
      ...
      const base64DataFilled = Onfido.byteArrayStringToBase64(
        "[104, 101, 108, 108, 111, 32, 76, 117, 107, 97, 115]"
      );
