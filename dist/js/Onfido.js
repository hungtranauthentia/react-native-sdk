...
import { OnfidoAlpha2CountryCode, OnfidoCaptureType, OnfidoCountryCode, OnfidoDocumentType } from "./config_constants";
...
const Onfido = {
    start(config) {
        ...
        if (config.workflowRunId === undefined) {
            ...
            if (config.flowSteps.captureDocument) {
                ...
                if (config.flowSteps.captureDocument.allowedDocumentTypes &&
    ...
    addCustomMediaCallback(callback) {
    ...
    byteArrayStringToBase64(byteArrayString) {
        let charString = '';
